...
class PublicationImagesService {
  ...
  async getAvalibleImageOrders(publicationId) {
    ...
    if (images.length >= avalibleValues.length) throw new CustomError('Not available spots for images for this publication. First, remove a image', 409, 'No Spots Available')

    let existOrders = images.map((image) => image['order'])
    let avalibleSpots = avalibleValues.filter(spot => !existOrders.includes(spot))
