...
class PublicationsServices {
  ...
  async findAndCount(query, user_id) {
    ...
    if (reference_link)
      options.where.reference_link = {
        [Op.reference_link]: `%${reference_link}%`,
      };
    ...
    const totalPages =
      size === 0
        ? 1
        : Math.ceil(publications.count / (size ? size : publications.count));
    const startIndex =
      ((page ? page : 1) - 1) * (size ? size : publications.count);
    const endIndex = startIndex + Number(size ? size : publications.count);

    const results =
      page > totalPages ? [] : publications.rows.slice(startIndex, endIndex);
    return {
  ...
  async createPublication(obj, tags) {
    ...
    try {
      ...
      await models.Votes.create(
        { publication_id: newPublication.id, user_id: newPublication.user_id },
        { transaction }
      );
      if (tags) {
  ...
  async getPublicationOr404(id) {
    let publication = await models.Publications.scope('view_public').findByPk(
      id,
      { raw: true }
    );
    if (!publication)
  ...
  async getPublicationsByUserVotes(userId) {
    ...
    try {
      ...
      let publications = await this.findAndCount({
  ...
  async getUserPublications(userId, query) {
    const transaction = await models.sequelize.transaction();
    try {
      ...
      if (user) {
        ...
        const totalPages =
          size === 0
            ? 1
            : Math.ceil(
              publications.count / (size ? size : publications.count)
            );
        const startIndex =
          ((page ? page : 1) - 1) * (size ? size : publications.count);
        const endIndex = startIndex + Number(size ? size : publications.count);
        ...
        const results =
          page > totalPages
            ? []
            : publications.rows.slice(startIndex, endIndex);
        let pagination = {
          count: 0,
          totalPages: 0,
          currentPage: 0,
